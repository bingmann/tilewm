...
{
    //! the WM_SIZE_HINTS data structure
    xcb_size_hints_t m_data;
    ...
    {
        return (m_data.flags & XCB_ICCCM_SIZE_HINT_P_MIN_SIZE);
    }
    ...
    {
        if (has_min_size())
            return std::max<int32_t>(0, m_data.min_width);
        else
            return 0;
    }
    ...
    {
        if (has_min_size())
            return std::max<int32_t>(0, m_data.min_height);
        else
            return 0;
    }
    ...
    {
        return (m_data.flags & XCB_ICCCM_SIZE_HINT_P_MAX_SIZE);
    }
    ...
    {
        return (m_data.flags & XCB_ICCCM_SIZE_HINT_P_RESIZE_INC);
    }
    ...
    {
        if (has_resize_inc())
            return std::max<int32_t>(1, m_data.width_inc);
        else
            return 1;
    }
    ...
    {
        if (has_resize_inc())
            return std::max<int32_t>(1, m_data.height_inc);
        else
            return 1;
    }
    ...
    void apply(uint16_t& width, uint16_t& height) const
    {
        ...
        if (m_data.flags & XCB_ICCCM_SIZE_HINT_P_SIZE)
        {
            basew = m_data.base_width;
            baseh = m_data.base_height;
            real_basew = basew;
            real_baseh = baseh;
        }
        else if (m_data.flags & XCB_ICCCM_SIZE_HINT_P_MIN_SIZE)
        {
            // base size is substituted with min size if not specified
            basew = m_data.min_width;
            baseh = m_data.min_height;
        }

        if (m_data.flags & XCB_ICCCM_SIZE_HINT_P_MIN_SIZE)
        {
            minw = m_data.min_width;
            minh = m_data.min_height;
        }
        else if (m_data.flags & XCB_ICCCM_SIZE_HINT_P_SIZE)
        {
            // min size is substituted with base size if not specified
            minw = m_data.base_width;
            minh = m_data.base_height;
        }

        // Handle the size aspect ratio
        if (m_data.flags & XCB_ICCCM_SIZE_HINT_P_ASPECT
            && m_data.min_aspect_den > 0
            && m_data.max_aspect_den > 0
            && height > real_baseh
            && width > real_basew)
        {
            ...
            double ratio = dx / dy;
            double min = m_data.min_aspect_num / (double)m_data.min_aspect_den;
            double max = m_data.max_aspect_num / (double)m_data.max_aspect_den;
        ...
        if (m_data.flags & XCB_ICCCM_SIZE_HINT_P_MAX_SIZE)
        {
            if (m_data.max_width)
                width = std::min<int32_t>(width, m_data.max_width);

            if (m_data.max_height)
                height = std::min<int32_t>(height, m_data.max_height);
        }

        // Handle the size increment
        if (m_data.flags & (XCB_ICCCM_SIZE_HINT_P_RESIZE_INC |
                            XCB_ICCCM_SIZE_HINT_BASE_SIZE)
            && m_data.width_inc && m_data.height_inc)
        {
            uint16_t t1 = (basew < width ? width - basew : 0);
            uint16_t t2 = (baseh < height ? height - baseh : 0);
            width -= t1 % m_data.width_inc;
            height -= t2 % m_data.height_inc;
        }
    }
